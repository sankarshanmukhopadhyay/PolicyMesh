...
PolicyMesh continues to default to a filesystem-first storage model. For operators who want a more durable single-node or small-team deployment profile, `v0.13.0` adds an optional SQLite backend.
...
Then run PolicyMesh as usual.

## What is stored in SQLite

When the SQLite backend is enabled, PolicyMesh writes structured records for:
